...
export function combineArrays2(array_of_arrays) {
    ...
    let newCombination = formCombination2(odometer, array_of_arrays);

    output.push(newCombination);

    while (odometer_increment(odometer, array_of_arrays)) {
        newCombination = formCombination2(odometer, array_of_arrays);
        output.push(newCombination);
    }
...
// Translate "odometer" to combinations from array_of_arrays
function formCombination2(odometer, array_of_arrays) {
    // In Imperative Programmingese (i.e., English):
    // let s_output = "";
    // for( let i=0; i < odometer.length; i++ ){
    //    s_output += "" + array_of_arrays[i][odometer[i]]; 
